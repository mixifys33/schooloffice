...
function ClassTeacherCAEntryPageContent() {
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hasChanges && !isUnmountingRef.current) {
        ...
        if (activeCaId && editedScores.size > 0) {
          ...
          navigator.sendBeacon('/api/teacher/assessments/ca/scores', blob)
        }
  ...
  const handleSubmitFinal = async () => {
    ...
    try {
      ...
      if (hasChanges && editedScores.size > 0) {
        ...
        const saveResponse = await fetch('/api/teacher/assessments/ca/scores', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            caId: activeCaId,
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>CA Scores - ${activeCa.name}</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="header-left">
              <h1>${activeCa.name} - ${caData.subject.name}</h1>
              <div class="info">${caData.class.name}</div>
            </div>
          ...
          <div class="details">
            <div><strong>Max Score:</strong> ${activeCa.maxScore}</div>
            <div><strong>Date:</strong> ${new Date(activeCa.date).toLocaleDateString('en-UG')}</div>
            <div><strong>Total Students:</strong> ${activeCa.studentScores.length}</div>
            <div><strong>Type:</strong> ${activeCa.type}</div>
          </div>
  ...
  const handleDownloadCSV = () => {
    ...
    const csvContent = [
      ...
      '',
      `CA Entry: ${activeCa.name}`,
      `Subject: ${caData.subject.name}`,
      `Class: ${caData.class.name}`,
      `Max Score: ${activeCa.maxScore}`,
      `Type: ${activeCa.type}`,
      `Date: ${new Date(activeCa.date).toLocaleDateString('en-UG')}`,
      `Total Students: ${activeCa.studentScores.length}`,
    ...
    link.setAttribute('download', `CA_${activeCa.name}_${caData.class.name}_${new Date().toISOString().split('T')[0]}.csv`)
