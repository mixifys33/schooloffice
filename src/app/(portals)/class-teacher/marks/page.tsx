...
import { ArrowLeft, BookOpen, AlertCircle, Save, Send, Lock, CheckCircle, Info } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { SkeletonLoader } from '@/components/ui/skeleton-loader'

/**
 * Marks Entry Page for Class Teacher Portal
...
export default function ClassTeacherMarksEntryPage() {
  ...
  useEffect(() => {
    async function fetchAssignedClasses() {
      try {
        const response = await fetch('/api/class-teacher/marks/classes')
        if (!response.ok) {
  ...
  useEffect(() => {
    async function fetchExams() {
      ...
      try {
        const response = await fetch(`/api/class-teacher/marks/exams?classId=${selectedClassId}`)
        if (!response.ok) {
    ...
    async function fetchMarksData() {
      ...
      try {
        const response = await fetch(
          `/api/class-teacher/marks?classId=${selectedClassId}&subjectId=${selectedSubjectId}&examId=${selectedExamId}`
        )
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/class-teacher/marks', {
        method: 'POST',
  ...
  const handleSubmitFinal = async () => {
    ...
    try {
      ...
      if (hasChanges) {
        ...
        await fetch('/api/class-teacher/marks', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            examId: selectedExamId,
      ...
      // Submit final marks
      const response = await fetch('/api/class-teacher/marks/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          examId: selectedExamId,
