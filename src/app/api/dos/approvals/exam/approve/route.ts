...
/**
 * POST /api/dos/approvals/exam/approve
 * Approve all exam entries for a class-subject-term
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Update all SUBMITTED exam entries for this class-subject-term
    const result = await prisma.examEntry.updateMany({
      where: {
        termId,
        subjectId,
        student: { classId },
    ...
    // Update DosApproval to set examApproved to true
    if (dosApproval) {
      await prisma.dosApproval.update({
        where: { id: dosApproval.id },
        data: { examApproved: true },
      });
    } else {
    ...
    return NextResponse.json({
      success: true,
      message: "All exam entries approved successfully",
      updatedCount: result.count,
    });
  } catch (error) {
    console.error("❌ [API] /api/dos/approvals/exam/approve - Error:", error);
    return NextResponse.json(
      { error: "Failed to approve exam entries" },
      { status: 500 }
    );
  }
}
