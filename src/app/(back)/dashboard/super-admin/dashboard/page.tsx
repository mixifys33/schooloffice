...
export default function SuperAdminDashboard() {
  ...
  const announcer = useRef<{ announce: (message: string) => void }>({
  ...
  const closeBulkActionDialog = useCallback(() => {
    ...
    if (bulkActionsRef.current) {
      const firstFocusable = bulkActionsRef.current.querySelector('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])') as HTMLElement
      ...
      render: (_, row) => (
        ...
        >
          <button
            ...
            onKeyDown={keyboardNavigation.handleEnterSpace(() => handleSelectSchool(row.id))}
          >
