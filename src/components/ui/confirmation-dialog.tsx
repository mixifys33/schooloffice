...
export function ConfirmationDialog({
  ...
  const config = actionType && actionType !== 'custom' ? actionConfigs[actionType] : null
  ...
  const iconColor = config?.iconColor || 'text-[var(--accent-primary)]'
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            ...
            'bg-[var(--bg-main)] dark:bg-[var(--bg-main)] rounded-lg shadow-lg',
            'data-[state=open]:animate-in data-[state=closed]:animate-out',
            'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
        ...
        >
          ...
          <DialogPrimitive.Close
            className={cn(
              ...
              'hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--border-default)]',
          ...
          <div className="p-6">
            ...
            <div className="flex items-start gap-4">
              <div className={cn(
                ...
                actionType === 'delete' || actionType === 'suspend-account'
                  ? 'bg-[var(--danger-light)] dark:bg-[var(--danger-dark)]/30'
                  : actionType
                  ? 'bg-[var(--warning-light)] dark:bg-[var(--warning-dark)]/30'
                  : 'bg-[var(--info-light)] dark:bg-[var(--info-dark)]/30'
              )}>
                <Icon className={cn('h-6 w-6', iconColor)} />
              </div>
              <div className="flex-1">
                <DialogPrimitive.Title className="text-lg font-semibold text-[var(--text-primary)] dark:text-[var(--text-primary)]">
                  {displayTitle}
                </DialogPrimitive.Title>
                {itemName && (
                  <p className="mt-1 text-sm font-medium text-[var(--text-primary)] dark:text-[var(--text-muted)]">
                    {itemName}
                  </p>
                )}
              </div>
            ...
            {/* Description */}
            {displayDescription && (
              <DialogPrimitive.Description className="mt-4 text-sm text-[var(--text-secondary)] dark:text-[var(--text-muted)]">
                {displayDescription}
            ...
            {/* Warning Message */}
            {warningMessage && (
              <div className="mt-4 rounded-md bg-[var(--warning-light)] dark:bg-[var(--warning-dark)]/20 p-3 border border-amber-200 dark:border-amber-800">
