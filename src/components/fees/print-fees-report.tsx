...
interface PrintFeesReportProps {
...
export function PrintFeesReport({ students, summary, schoolName, printType }: PrintFeesReportProps) {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        borderBottom: isColored ? '4px solid #3b82f6' : '2px solid #000',
        paddingBottom: '20px',
        marginBottom: '30px',
        display: 'flex',
      ...
      <div style={{
        ...
        <div style={{
          padding: '16px',
          border: isColored ? '2px solid #3b82f6' : '1px solid #ddd',
          borderRadius: '8px',
          backgroundColor: isColored ? '#eff6ff' : '#f9fafb'
        }}>
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Total Students</div>
          <div style={{ fontSize: '24px', fontWeight: 'bold', color: isColored ? '#1e40af' : '#000' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '12px', color: '#666', marginBottom: '4px' }}>Paid Students</div>
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <table style={{
          ...
          <thead>
            <tr style={{
              backgroundColor: isColored ? '#3b82f6' : '#f3f4f6',
          ...
          <tbody>
            {students.map((student, index) => (
              <tr key={student.id} style={{
                ...
                <td style={{ padding: '10px 8px', borderBottom: '1px solid #e5e7eb' }}>
                  {student.className} {student.streamName ? `(${student.streamName})` : ''}
                </td>
                ...
                <td style={{ padding: '10px 8px', borderBottom: '1px solid #e5e7eb', textAlign: 'center' }}>
                  <span style={{
                    ...
                    backgroundColor: isColored ? (
                      student.paymentStatus === 'PAID' ? '#d1fae5' :
                      student.paymentStatus === 'PARTIAL' ? '#fef3c7' : '#fee2e2'
                    ) : '#f3f4f6',
                    color: getStatusColor(student.paymentStatus)
                  }}>
                    {student.paymentStatus === 'PAID' ? 'Paid' :
                     student.paymentStatus === 'PARTIAL' ? 'Partial' : 'Not Paid'}
                  </span>
                </td>
      ...
      {/* Footer */}
      <div style={{
        borderTop: isColored ? '2px solid #3b82f6' : '1px solid #ddd',
        paddingTop: '20px',
        marginTop: '40px',
        display: 'flex',
        justifyContent: 'space-between',
        ...
        <div>
          <strong>Summary:</strong> {summary.paidStudents} paid, {summary.partialStudents} partial, {summary.unpaidStudents} unpaid
        </div>
        <div style={{ textAlign: 'right' }}>
          <div>Powered by <strong style={{ color: isColored ? '#3b82f6' : '#000' }}>SchoolOffice.academy</strong></div>
